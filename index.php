...
require_once(dirname(__FILE__) . "/classes/process_sub_nav_link_factory.class.php");
...
function main() { // C! :D
    try {
        ...
        foreach (DBIF::get()->get_process_actions() as $action) {
            $actions[] = $action; // the NavLinkFactory wants these
            $process_actions[] = $action; // the ViewFactory wants these
        }
        $actions[] = "contact";
        $actions[] = "guestbook";
        ...
        try { // a nested try-block for a fancy exception view, since we can do that now after the inits
            ...
            Views\ViewFactory::get()->get_view($request["action"], $request["params"], $request["language"], $nlf, $gallery_actions, $process_actions)->render();
        } catch (Exception $e) {
            $is_ajax = (isset($_REQUEST["is_ajax"]) ? $_REQUEST["is_ajax"] : false);
            $view = new Views\ExceptionView(array("exception" => $e, "is_ajax" => $is_ajax), $nlf, $gallery_actions, $process_actions);
            $view->render();
        }
    } catch (Exception $e) {
        if (!headers_sent()) {
